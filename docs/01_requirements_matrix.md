...
│   └── prds/{PRD-001..007-*.md, PRD_INDEX.md, catalog/PRD-0001..0522.md}
├── tools/                       # generate_prd_catalog.py, prd_catalog_data.json
├── data/
├── results/
├── assets/
└── outputs/
...
| HW-F02 | NL decode + inference + move translation | `services/agents/`, PRD-003 | 4,5,13 | ✅ (full pipeline: decode → infer → **translate to move**, verified with a real LLM — `BaseAgent` tracks a running belief; the configured `DecisionStrategy` (default: `MinimaxDecisionStrategy`) acts on it via `make_belief_state`, falling back through `HeuristicStrategy`'s chase/flee otherwise; confirmed live: Cop correctly parsed `(4,4)` from the Thief's free-text message) |
| HW-F03 | Success = communication ability, not win-rate | PRD-003 acceptance criteria, `docs/05_testing_strategy.md` | 5,13 | ✅ (decision quality is now config-selectable — `decision_strategy: "heuristic"\|"llm"\|"minimax"` — so the simple/deterministic option from earlier chunks remains available; success criteria still center on the MCP communication pipeline itself, not on either agent's win-rate; README §2 documents this) |
...
| HW-F16 | 3-stage rollout: local → cloud → bonus competition | `docs/06_submission_checklist.md`, PRD-007, `Dockerfile`, `tools/deploy_cloud_run.sh` | 10,14 | 🟨 (Stage 1 local: ✅ done and proven; Stage 2 cloud: code/container/deploy-script done and ✅, actual deployment requires the user's own GCP account — not something I can perform autonomously; Stage 3 bonus: external, HW-Q06) |
| HW-F17 | Token auth + revoke, no open firewall exposure | `services/mcp/server_base.py`, `services/mcp/run_server.py` | 3,14 | ✅ (local auth+revoke proven; `run_server.py` refuses to start a deployed server without a real `MCP_COP_AUTH_TOKEN`/`MCP_THIEF_AUTH_TOKEN` secret — no default/placeholder token in production; Cloud Run's platform-level `--allow-unauthenticated` is intentional, since HW-F17's auth is enforced at the application layer via `TokenAuthManager`, not Cloud IAM) |
| HW-F18 | Exactly 2 URLs per group (Cop, Thief) | `.env-example`, `sdk/wiring.py::build_clients_from_env` | 3,14 | ✅ (`MCP_COP_URL`/`MCP_THIEF_URL` now genuinely read and used — `build_clients_from_env` connects to them when both are set, falling back to in-process otherwise; `tools/deploy_cloud_run.sh` prints the real URLs after a live deploy) |
| HW-F19 | 3 LLM connectivity architectures, pluggable | `services/agents/llm_client.py` (LLMClient interface), `services/agents/llm_providers.py` | 4 | 🟨 (Architecture #1, public cloud API: **Anthropic now implemented and tested** via `AnthropicCompleteFn` + `wiring.build_llm_client_from_env`; OpenAI/Gemini not yet added; Ollama+tunnel/hybrid architectures #2/#3 remain Chunk 10) |
...
| HW-F27 | Inter-group bonus game execution (external) | PRD-007, `docs/06_submission_checklist.md` | 11 (manual) | ⬜ **N/A for this codebase alone** — requires pairing with a second student group within 1 week of publication (HW-Q06); the reporting schema/scoring this requires is fully implemented (HW-F24/F28) and ready |
...
| SG-P02 | SDLC stage order | `docs/04_implementation_chunks.md` | all | ✅ (requirements → architecture → TDD → validation → deployment docs → maintenance, followed visibly chunk by chunk) |
...
| SG-C01 | ≤150 lines/file | all `src/` files, enforced via review + script | all | ✅ (superseded by the stricter PROJ-R01 physical-line check, `tests/test_line_limits.py`, passing for all 11 chunks) |
| SG-C02 | Docstrings, naming, SRP, DRY | all `src/` files | all | ✅ (verified by code review every chunk; every public class/function documents purpose, several explicitly document Input/Output/Setup per SG-C13) |
...
| SG-C08 | No magic values | `constants.py`, `config/` | all | ✅ (grid/scoring/rate-limit literals all sourced from `constants.py` or `config/`; verified by code review) |
...
| SG-C14 | ISO/IEC 25010 self-check | `docs/06_submission_checklist.md` | 11 | ✅ (self-assessment performed and recorded in `docs/06_submission_checklist.md` §H, Chunk 11) |
...
| SG-U03 | Git workflow (branches, PRs, tags) | process, not a file — tracked in `docs/06_submission_checklist.md` | all | 🟨 (commit messages meaningful, one per chunk; **honest gap**: no feature branches/PRs/tags — all work committed directly to `main` per the user's explicit per-chunk workflow request, see `docs/06_submission_checklist.md` §D) |
| SG-U04 | Prompt Engineering Log | `docs/08_claude_work_log.md` (doubles as the log) | 1 | ✅ (11 entries covering every chunk, including self-corrections and the fresh-clone bug fix) |
