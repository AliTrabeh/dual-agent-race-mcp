...
## Entry 15 — 2026-06-26 — Chunk 12: LLM-driven decision strategy

**Prompt context/goal**: After seeing "why does the Cop always win?" explained as a geometry artifact, the user rejected that as sufficient: "but why thief take fixed steps? i dont want that... no, i want also to think in which way they move." This was a request to change the actual decision algorithm, not just explain the existing one better.

**What was done**:
1. Added `services/agents/strategies/llm_strategy.py` (`LLMDecisionStrategy`): asks the LLM to pick the next action directly (UP/DOWN/LEFT/RIGHT/PLACE_BARRIER), strictly parses the reply, and falls back to `HeuristicStrategy` on any failure, ambiguity, or illegal choice — the no-network stub still behaves exactly as before (it never produces a parseable action either, so every existing test stayed valid).
2. Wired it as the new default in `sdk/wiring.py::build_agents`.
3. Full test coverage for parsing success/failure/fallback paths.

**Key decision**: this was an intermediate step on the way to Chunk 13's minimax — the user's next message asked for genuine adversarial search ("I want to give each agent the best strategy to maximize the win probability"), which `LLMDecisionStrategy` alone doesn't provide (LLM judgment isn't lookahead). Kept as a config option (`decision_strategy: "llm"`) rather than deleting it.

## Entry 16 — 2026-06-26 — Chunk 13: minimax + alpha-beta competitive decision strategy

**Prompt context/goal**: User gave a large, fully-detailed spec: replace fixed/LLM-judgment decisions with real adversarial search for both agents — depth-limited minimax + alpha-beta pruning + heuristic evaluation + safe fallback, with the Cop as a "minimax trapper" and the Thief as a "minimax survivor," built on shared BFS/mobility/reachable-area/trap-risk utilities. Explicit instruction: "Do not explain or reimplement the game rules or scoring. The game already works."

**What was done**:
1. New `services/agents/strategies/minimax/` subpackage, each file under the 150-line cap:
   - `board_utils.py` — `clone_state` (independent RaceState copy for search, never mutates the real state), `legal_actions`, `state_hash`, `make_belief_state` (builds a RaceState from one agent's own legitimate knowledge: true self-position + real barriers + current best *estimate* of the opponent — search never touches engine ground truth, resolving the Dec-POMDP-vs-minimax tension).
   - `search_metrics.py` — BFS `bfs_distance`/`reachable_area`, role-aware wrappers (`distance_for_role`, `reachable_area_for_role`, `mobility_for_role`), `cop_threat_positions`/`is_safe_for_thief` for Thief safety checks.
   - `features.py` / `evaluation.py` — `compute_features` (shared raw numbers) feeding `evaluate_for_cop`/`evaluate_for_thief` (terminal states scored via the real `RaceState.check_outcome()`, non-terminal states via tunable weights).
   - `move_ordering.py` — immediate-capture-first for the Cop, distance-increasing-first for the Thief, to improve alpha-beta pruning.
   - `transposition.py` — a simple per-decision transposition table keyed on (Cop position, Thief position, barriers, barriers placed, move count, depth).
   - `search.py` — `minimax_decide`: depth-limited minimax with alpha-beta pruning and a time budget; returns `None` only if no action could be scored before the deadline.
   - `fallback.py` — one-ply heuristic fallback using the same evaluation functions, for when search times out or raises.
   - `strategy.py` — `MinimaxDecisionStrategy`: tracks a running opponent-position estimate (refined from `believed_opponent_position` when NL inference succeeds, defaulting to the diagonally-opposite corner on the first call), calls `minimax_decide`, falls back to `fallback_action` on `None` or any exception — never crashes, never returns an illegal action.
2. Extended `AgentObservation` with `barriers`/`move_count`/`max_moves`/`max_barriers` (all defaulted) — physical/rule facts, not hidden state, needed for mobility/reachable-area computation without breaking partial observability.
3. Made `decision_strategy` genuinely config-driven (`sdk/wiring.py::_build_strategy`): `"minimax"` (new default), `"llm"`, or `"heuristic"` — never hard-coded, each agent gets its own strategy instance (minimax carries per-agent belief state that must not be shared).
4. Added `RandomDecisionStrategy` (a pure baseline, never used by `build_agents`) and `tools/simulate_strategies.py` — a standalone benchmark that plays strategies against each other directly on `RaceState` (bypassing MCP/LLM/NL-inference, to isolate search quality from inference noise) and reports Cop win rate, Thief survival, average capture turn, barriers used, and illegal actions. Results: Minimax Cop beats Heuristic/Random Thief in ~6 turns (3×3) / ~14 turns (5×5) with zero illegal actions; a Heuristic or Random Cop *never* catches a Minimax Thief (the lookahead lets it evade indefinitely) — confirming the upgrade meaningfully changes outcomes for both roles, not just the Cop.
5. Full test suite for every new module (`tests/.../test_minimax/`), including white-box tests of the private `_search` recursion to deterministically hit the timeout-propagation and no-legal-actions branches. 320/320 tests, 100% coverage, 0 ruff warnings.

**Key decisions / honest findings**:
- **Barriers are essentially useless for the Cop under the real game rule.** The user's spec assumed barriers reduce Thief mobility/reachable area; the actual engine rule (HW PDF, Chunk 5, already tested) is that barriers block *only* the Cop, never the Thief. Kept the real rule rather than reimplementing it, and verified the evaluation functions correctly reflect this — a well-tuned Cop almost never places one, since it can only ever restrict its own movement. Flagged explicitly to the user rather than silently picking an interpretation; user did not ask to change the underlying barrier rule.
- **Live full-match testing with the no-network stub LLM does not exercise minimax's real strength**: the stub's `interpret_message` always returns "UNKNOWN," so `believed_opponent_position` never updates past the one-time default-corner guess for the whole match — both `HeuristicStrategy`'s chase/flee and `MinimaxDecisionStrategy` are equally limited by this, not a new defect. A live run with the real Anthropic LLM hit the pre-existing `config/rate_limits.json` "llm" service cap (20 req/min) mid-match — a known, previously-flagged open question, not introduced by this chunk. User chose to skip the live real-LLM demo for now and rely on the unit tests + the isolated ground-truth benchmark instead.
- Resolved the partial-observability-vs-minimax tension by having search always operate on the agent's own belief estimate via `make_belief_state`, never on `RaceState` ground truth — consistent with the Dec-POMDP framing established in the README since Chunk 1.

## Entry 17 — 2026-06-27 — Chunk 14: cloud deployment path (Google Cloud Run)

**Prompt context/goal**: User asked to start cloud deployment (HW PDF §6's Stage 2 of the 3-stage rollout, previously only an actionable guide in README §8, never actually built). `gcloud` was confirmed not installed locally, so I asked the user how to proceed; they chose Google Cloud Run and accepted that they'd need to run `gcloud auth login` themselves since I can't authenticate as them.

**What was done**:
1. Discovered `MCP_COP_URL`/`MCP_THIEF_URL`/`MCP_COP_AUTH_TOKEN`/`MCP_THIEF_AUTH_TOKEN` were already documented in `.env-example` since early chunks but never actually read anywhere in the codebase — a real gap, not something to silently leave.
2. `services/mcp/run_server.py` — new standalone entry point: builds one role's server (reusing `create_cop_server`/`create_thief_server` unchanged) and binds it to a real HTTP port via `FastMCP.run(transport="http", ...)`. Reads its auth token from `MCP_COP_AUTH_TOKEN`/`MCP_THIEF_AUTH_TOKEN` and refuses to start (`SystemExit`) without one — never a default/placeholder token in production (HW-F17). Verified live: started it locally bound to a real port with a fake local token, confirmed it responds over real HTTP, then stopped it — no real cloud calls.
3. `sdk/wiring.py::build_clients_from_env` — mirrors `build_llm_client_from_env`'s established pattern: builds `AgentMCPClient`s pointed at `MCP_COP_URL`/`MCP_THIEF_URL` when both are set, otherwise falls back to in-process `build_clients` — deployment becomes a pure config/env change, never a code change.
4. `Hw6RaceSDK.run_match()` — new method using `build_clients_from_env`; kept the existing `run_local_match()` unchanged (always in-process, used by Stage-1-specific tests) rather than silently changing its behavior, since "local" is this project's established term for in-process (README's Stage 1/2/3 language) — overloading it would have been confusing. `main.py` now calls `run_match()`.
5. `Dockerfile` + `.dockerignore` — one shared image for both roles; `--role` is supplied per Cloud Run service at deploy time (`--args`), not baked into the image, so one build serves both servers.
6. `tools/deploy_cloud_run.sh` — builds and pushes the image via Cloud Build, deploys both Cloud Run services (generating random auth tokens via `openssl rand` if none are already set), and prints the two real URLs/tokens for the user to add to their own `.env`. Not run by me — requires the user's own GCP project/billing.
7. Full test suite for everything testable without a live cloud account: `run_server.py`'s token-validation/server-building/arg-parsing/`main()` logic (with `server.run()` mocked), `build_clients_from_env`'s fallback/remote-token logic, and a `run_match()` end-to-end integration test confirming it behaves identically to `run_local_match()` when no remote URLs are configured.
8. `run_server.py` added to `[tool.coverage.run] omit`, matching the existing precedent for `main.py` — both are process entry points whose untestable `if __name__ == "__main__":` guard shouldn't force lower meaningful coverage elsewhere; their actual logic is still unit-tested via direct function calls.
9. `test_wiring.py` exceeded the 150-line cap after the new tests were added — split the new remote-clients tests into `test_wiring_remote.py` rather than trimming test coverage.
10. 342/342 tests, 100% coverage, 0 ruff warnings.
